...
def knn(test_point, k): #Create dummy target variable
for the KNN Classifier
target = [0
    for item in movie_titles
]# Instantiate object
for the Classifier
model = KNearestNeighbours(data, target, test_point, k = k)# Run the algorithm
model.fit()# Distances to most distant movie
max_dist = sorted(model.distances, key = itemgetter(0))[-1]# Print list of 10 recommendations <
    Change value of k
for a different number >
    table = list()
for i in model.indices: #Returns movie title and imdb link
...
menu = ["Home", "Login", "SignUp", "About", "Contribute",
    "Feedback form"
]
...
st.markdown(""
    " <style> .font {
    font - size: 30 px; font - family: 'Trebuchet MS'; color: #
    C0C0C0; text - align: center;
} < /style> """, unsafe_allow_html=True)
st.markdown(
    '<p class="font">Hey movie geek, We hope you got your popcorns ready!</p>',
    unsafe_allow_html = True)
...
if st.sidebar.checkbox("Login"): #if password == '12345':
    create_usertable()
...
if app_options == 'Movie based':
    movie_select = st.selectbox('Select movie:', ['--Select--'] +
        movies)
if movie_select == '--Select--':
    st.write('Select a movie')
else :
    n = st.number_input('Number of movies:', min_value = 5, max_value =
        20, step = 1)
genres = data[movies.index(movie_select)]
test_point = genres
table = knn(test_point, n)
for movie, link in table: #Displays movie title with link to imdb
...
st.success("Added {} To Recent Searches".format(new_title1))
    ...
    st.error('No movie Found')
if choice1 == "Read": #st.subheader("View Items")
...
st.markdown('<p class="font">Create New Account</p>',
    unsafe_allow_html = True)
...
st.markdown(""
    " <style> .font2 {
    font - size: 20 px; font - family: 'Trebuchet MS'; color: #
    C0C0C0;
} < /style> """, unsafe_allow_html=True)
st.markdown(
    '<p class="font2" >This is a movie recommendation engine built as a part of Microsoft engage mentorship programme. It uses -- API to fetch data from IMdb and recommends movies to the user based on their input of genre or movie. '
...
st.markdown(
    ...
    unsafe_allow_html = True)
elif choice == "Contribute": #Add a file uploader to allow users to upload their project plan file
st.markdown(""
    " <style> .font {
    font - size: 30 px; font - family: 'Trebuchet MS'; color: #
    FD1C03; text - align: center;
} < /style> """, unsafe_allow_html=True)
st.markdown('<p class="font">Wanna Contribute In our Project ? </p>',
    unsafe_allow_html = True)
new_title = '<br></br>'
st.markdown(new_title, unsafe_allow_html = True)
st.markdown(""
    " <style> .font21 {
    font - size: 20 px; color: White; text - align: center;
} < /style> """, unsafe_allow_html=True)
st.markdown(
    '<p class="font21">Are we missing a favorite of your movies in our database? Dont Worry  we had '
...
raw_text = str(docx_file.read(), "utf-8")# works with st.text and st.write,
    used
...
st.markdown('<p class="font">Fill up the feedback form</p>',
    unsafe_allow_html = True)
with st.form(key = 'columns_in_form2',
        clear_on_submit = True): #set clear_on_submit = True so that the form will be reset /
    cleared once it 's submitted#
